...
elif [[ -n "$2" && "$2" == "make" ]];then
    CMAKE_ARG_BUILD_TOOL_TYPE_CONFIG="-G Unix Makefiles"
elif [[ -n "$2" && "$2" == "ninja" ]];then
    CMAKE_ARG_BUILD_TOOL_TYPE_CONFIG="-G Ninja"
else
...
export CC=clang
export CXX=clang++
cmake -S . -B build "${CMAKE_ARG_BUILD_TYPE_CONFIG}" "${CMAKE_ARG_BUILD_TOOL_TYPE_CONFIG}" -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_POLICY_VERSION_MINIMUM=3.5
